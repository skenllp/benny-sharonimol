/* ============================================================
   BENNY & SHARONIMOL — WEDDING INVITATION
   ============================================================ */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---------- LIVE COUNTDOWN ---------- */
  // Wedding: 2 November 2026, ceremony 11:00 AM IST
  const weddingDate = new Date('2026-11-02T11:00:00+05:30').getTime();
  ...
  if (ceremonyLink){
    ceremonyLink.href = 'https://www.google.com/maps/search/?api=1&query=' + encodeURIComponent('St. George Church, Marykulam, Idukki, Kerala');
  }
  if (receptionLink){
    receptionLink.href = 'https://www.google.com/maps/search/?api=1&query=' + encodeURIComponent("St. Mary's Church, HRC9+46R, Kottayam-Kumily Rd, Podimattam, Kanjirappally, Kottayam, Kerala 686512");
  }
  ...
  function loadComments(){
    let list = [];
    try { list = JSON.parse(localStorage.getItem('bs_comments') || '[]'); } catch(err){ list = []; }
  ...
  if (commentForm){
    commentForm.addEventListener('submit', (e) => {
      ...
      try { localStorage.setItem('bs_comments', JSON.stringify(list)); } catch(err){ /* storage unavailable */ }
